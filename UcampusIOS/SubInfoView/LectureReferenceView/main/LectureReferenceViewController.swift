...
class LectureReferenceViewController: UIViewController, SubViewControllerWithTable {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? BBSPostViewController {
            ...
            destination.isNoticeView = false
        }
    }
    
    func fetchAll(for page: Int) {
        let url = Urls.reference.rawValue + container.lecture.referenceQuery  + "&p_pageno=" + String(page)
    ...
    override func viewDidLoad() {
        ...
        let url = Urls.reference.rawValue + container.lecture.referenceQuery  + "&p_pageno=0"
        Alamofire.request(url).response() { response in
            if let data = response.data,
                let html = String(data: data, encoding: .utf8) {
